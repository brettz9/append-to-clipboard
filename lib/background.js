...
async function updateContextMenus () {
  await Promise.all(contextMenus.map(async ([
    menuID, menuI18nKey, {contexts} = {contexts: ['selection', 'link']}
  ]) => {
...
browser.contextMenus.onClicked.addListener(
  async ({menuItemId, linkText, linkUrl}, tab) => {
    ...
    await browser.scripting.executeScript({
      ...
      func (mnuItemId, lnkText, lnkUrl) {
        globalThis.appendToClipboard(mnuItemId, lnkText, lnkUrl);
      },
      injectImmediately: false
    });
  }
